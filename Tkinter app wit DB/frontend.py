...
def add_command():
    backend.insert(text_title.get(), text_author.get(),int(text_year.get()), int(text_isbn.get()))
    list1.delete(0, END)
    list1.insert(END, (text_title.get(), text_author.get(),int(text_year.get()), int(text_isbn.get())))

def delete_command():
    backend.delete(selected_tuple[0]) # accessing a global variable
    clear_entry()
    list1.delete(0,END)
    list1.insert(END,selected_tuple)

def update_command():
    backend.update(selected_tuple[0], text_title.get(), text_author.get(), text_year.get(), text_isbn.get())
    list1.delete(0, END)
